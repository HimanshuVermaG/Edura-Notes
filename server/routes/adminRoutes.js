...
// GET /api/admin/users - list users with note count
router.get('/users', async (req, res) => {
  try {
    const users = await User.find().select('name email createdAt role').sort({ name: 1 }).lean();
    const counts = await Note.aggregate([{ $group: { _id: '$userId', count: { $sum: 1 } } }]);
    ...
    const list = users.map((u) => ({
      ...
      role: u.role,
      noteCount: countByUser[String(u._id)] ?? 0,
    }));
    res.json(list);
  } catch (err) {
...
// GET /api/admin/users/:userId - single user with notes
